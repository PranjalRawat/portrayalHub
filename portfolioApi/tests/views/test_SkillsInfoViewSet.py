...
class SkillsInfoViewSetTest(APITestCase):
    ...
    def test_user_skills_info_list_view(self):
        ...
        self.assertEqual(len(SkillsInfoSerializer(SkillsInfoModel.objects.all(), many=True).data), len(response.data))

        serialized_data = SkillsInfoSerializer(SkillsInfoModel.objects.all(), many=True).data
