...
from portfolioApi.models import CertificateInfoModel, UserProfileModel
from portfolioApi.serializers import CertificateInfoSerializer
...
# Test Execution Command :::>>>  python3 manage.py test portfolioApi.tests.views.test_CertificateInfoViewSet
class CertificateInfoViewSetTest(APITestCase):

    def setUpUrl(self):
        self.client.login(**self.user_data)
        self.pk = self.client.get(self.list_url).data[0]['id']
        self.detail_url = reverse('certificatesInfo-detail', kwargs={'pk': self.pk})

    def setUp(self):
        self.client = APIClient()
        self.media_fields = ['course_badge']
        self.list_url = reverse('certificatesInfo-list')
        ...
        self.upload_dir = 'certificates_badge'
        ...
        for ins in range(3):
            CertificateInfoModel.objects.create(**{
    ...
    def test_user_certificate_info_detail_view(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        serialized_data_item = CertificateInfoSerializer(CertificateInfoModel.objects.get(id=self.pk)).data
    ...
    def test_unauthorized_user_cannot_post_data(self):
        ...
        certificateInfo = {
            ...
            'issuing_organization': fake.company(),
            'issue_date': fake.date(),
            'expiration_date': fake.date_between_dates().strftime('%Y-%m-%d'),
            'credential_url': fake.url(),
            'course_badge': self.image_file,
            'featured': fake.boolean(),
        }
        response = self.client.post(self.list_url, certificateInfo)
    ...
    def test_unauthorized_user_cannot_put_data(self):
        ...
        response = self.client.put(self.detail_url, certificateInfo)
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
    ...
    def test_unauthorized_user_cannot_patch_data(self):
        ...
        response = self.client.patch(self.detail_url, certificateInfo)
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
