...
from portfolioApi.models import MajorProjectsInfoModel, UserProfileModel
from portfolioApi.serializers import MajorProjectsInfoSerializer
...
# Test Execution Command :::>>>  python3 manage.py test portfolioApi.tests.views.test_MajorProjectsInfoViewSet
class ProjectsInfoViewSetTest(APITestCase):

    def setUpUrl(self):
        self.client.login(**self.user_data)
        self.pk = self.client.get(self.list_url).data[0]['id']
        self.detail_url = reverse('projectsInfo-detail', kwargs={'pk': self.pk})

    def setUp(self):
        self.client = APIClient()
        self.media_fields = ['project_logo']
        self.list_url = reverse('projectsInfo-list')
        ...
        for ins in range(5):
            MajorProjectsInfoModel.objects.create(**{
    ...
    def test_user_projects_info_list_view(self):
        self.client.login(**self.user_data)
        response = self.client.get(self.list_url)

        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(MajorProjectsInfoSerializer(MajorProjectsInfoModel.objects.filter(user_profile=self.userProfileInstance), many=True).data), len(response.data))

        serialized_data = MajorProjectsInfoSerializer(MajorProjectsInfoModel.objects.filter(user_profile=self.userProfileInstance), many=True).data
    ...
    def test_user_projects_info_detail_view(self):
        self.client.login(**self.user_data)
        response = self.client.get(self.detail_url)

        self.assertEqual(response.status_code, status.HTTP_200_OK)
        serialized_data_item = MajorProjectsInfoSerializer(MajorProjectsInfoModel.objects.get(id=self.pk)).data
    ...
    def test_unauthorized_user_cannot_post_data(self):
        ...
        projectsInfo = {
            ...
            'currently_working': fake.boolean(),
            'project_url': fake.url(),
            'project_logo': self.image_file,
            'featured': fake.boolean(),
        }
        response = self.client.post(self.list_url, projectsInfo)
    ...
    def test_unauthorized_user_cannot_put_data(self):
        ...
        response = self.client.put(self.detail_url, projectsInfo)
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
    ...
    def test_unauthorized_user_cannot_patch_data(self):
        ...
        response = self.client.patch(self.detail_url, projectsInfo)
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
