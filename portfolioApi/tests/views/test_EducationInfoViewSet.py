...
class EducationInfoViewSetTest(APITestCase):
    ...
    def test_user_education_info_list_view(self):
        ...
        self.assertEqual(len(EducationInfoSerializer(EducationInfoModel.objects.all(), many=True).data), len(response.data))

        serialized_data = EducationInfoSerializer(EducationInfoModel.objects.all(), many=True).data
